...
convert_medicos_cp <- function(x) {
  ...
  cols <- c("CDPOSTAL", "MSEGUIMIENTO", "MVALORACION", "MDICTAMEN PERICIAL", "MCONSIGNACION",
            "MOFERTA MOTIVADA", "MVALSECUIELAS", "M2ªOPINION", "MDICTAMEN SANIDAD",
            "MENCARGO COMISION", "MTELEVALORACION")
  if (!identical(cols,  names(medicos_cp))) stop ("Check columns first!.")
  
  ## Set column names to report types.
  colvars <- c("CP_ASIG", "SEGUIMIENTO", "VALORACION", "PERICIAL", "CONSIGNACION", 
               "OFERTA", "SECUELAS", "SEGUNDA", "SANIDAD", "COMISION", "TELEVALORACION")
  ...
  ## Melt table.
  medicos_melted <- melt(medicos_cp, id.vars = "CP_ASIG", measure.vars = 2:11, 
